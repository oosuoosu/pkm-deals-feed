...
for p in products:
    ...
    try:
        ...
        # Resize layout to be slightly wider and even shorter
        ...
        # Draw title (wrap every ~14 characters)
        title = p.get("title", "")
        lines = [title[i:i+12] for i in range(0, len(title), 12)][:4]
        for line in lines:
            draw.text((text_x, y_cursor), line, font=font_title, fill=(0, 0, 0))
            y_cursor += 40

        # Rating and reviews (with half-star support and cleaned 件)
